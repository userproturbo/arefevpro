...
export default function PhotoSceneHero({ children }: PhotoSceneHeroProps) {
  ...
  useEffect(() => {
    ...
    const translateZ = ((secondaryWave + 1) / 2) * 12;
    const scale = 1 + overshoot + idleWave * 0.005;
  ...
  }, [entryProgress, idleElapsedMs]);

  return (
    <section className="relative min-h-screen overflow-x-hidden bg-[#040404] lg:h-screen lg:overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_35%,rgba(255,220,160,0.14),transparent_32%),radial-gradient(circle_at_78%_18%,rgba(143,84,38,0.18),transparent_24%),linear-gradient(120deg,#050505_0%,#0b0806_48%,#040404_100%)]" />
      <div className="absolute inset-x-0 bottom-0 h-[28vh] bg-[linear-gradient(180deg,transparent_0%,rgba(6,4,3,0.45)_40%,rgba(13,8,5,0.94)_100%)]" />
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1680px] flex-col px-[clamp(24px,4vw,80px)] py-[clamp(20px,3vw,40px)] lg:h-screen lg:flex-row lg:items-center lg:justify-between lg:gap-10">
        <div className="relative flex min-h-[36vh] items-end justify-center overflow-visible lg:h-full lg:w-[min(55%,820px)] lg:flex-none lg:justify-start [perspective:1400px]">
          <div
            ...
            style={{ transform: "translate3d(0,0,0) scale(1)", transformStyle: "preserve-3d", willChange: "transform" }}
          >
            <Image
              ref={imageRef}
              ...
              className="h-auto max-h-[clamp(280px,65vh,680px)] w-full select-none object-contain"
            />
          </div>
        </div>
