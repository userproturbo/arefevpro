...
import PhotoCommentLikeButton from "@/app/components/photo/PhotoCommentLikeButton";
...
export default function PhotoComments({
  ...
  useEffect(() => {
    ...
    if (!initialComments || initialComments.length === 0 || !initialPagination) {
  ...
  const fetchRootComments = async ({
    ...
    try {
      ...
      const limit = pagination?.limit || initialPagination?.limit || 10;
      const res = await fetch(
        `/api/photos/${photoId}/comments?page=${page}&limit=${limit}`,
        {
          credentials: "include",
        }
      );
  ...
  const fetchReplies = async (
    ...
    try {
      ...
      const res = await fetch(`/api/photos/comments/${rootCommentId}/replies`, {
        credentials: "include",
      });
      if (!res.ok) throw new Error("failed");
      const data = await res.json();
  ...
  const submitReply = async (parentId: number) => {
    ...
    await requireUser(async () => {
      try {
        ...
        if (!res.ok) {
          const message = await readErrorMessage(res, "Не удалось отправить ответ");
          alert(message);
          return;
        }
        const data = await res.json();
  ...
  const deleteComment = async (id: number) => {
    ...
    try {
      ...
      const res = await fetch(`/api/photos/comments/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (!res.ok) throw new Error("failed");
