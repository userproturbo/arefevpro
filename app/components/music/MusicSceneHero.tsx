...
export default function MusicSceneHero({ children }: MusicSceneHeroProps) {
  ...
  useEffect(() => {
    ...
    const translateZ = ramp * (18 + 24 * (secondaryPulse + 1) * 0.5);
    const scaleOvershoot = 1 + (1 - entryProgress) * 0.07;
    const rhythmicScale = 1 + pulse * (0.01 + 0.012 * ramp * hoverBoost);
    const scale = idleElapsedMs <= 0 ? scaleOvershoot : rhythmicScale;
    const brightness = 1 + ramp * (isHovered ? 0.08 : 0.03);
    ...
    character.style.filter = `brightness(${brightness.toFixed(3)})`;
  }, [entryProgress, idleElapsedMs, isHovered]);

  return (
    <section className="relative min-h-screen overflow-x-hidden bg-[#060507] lg:h-screen lg:overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_28%,rgba(255,132,90,0.16),transparent_28%),radial-gradient(circle_at_78%_18%,rgba(255,207,98,0.1),transparent_24%),linear-gradient(120deg,#070507_0%,#140b11_45%,#060507_100%)]" />
      <div className="absolute inset-x-0 bottom-0 h-[30vh] bg-[linear-gradient(180deg,transparent_0%,rgba(14,7,10,0.5)_34%,rgba(9,5,7,0.96)_100%)]" />
        ...
        <div className="relative flex min-h-[38vh] items-center justify-center overflow-visible lg:h-full lg:w-[min(56%,860px)] lg:flex-none lg:justify-start [perspective:1600px]">
          ...
          >
            <Image
              ...
              className="h-auto max-h-[clamp(320px,70vh,720px)] w-full select-none object-contain"
            />
          </div>
        </div>
