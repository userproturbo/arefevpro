...
export default function DroneSceneHero({ children }: DroneSceneHeroProps) {
  ...
  useEffect(() => {
    ...
    const idleTranslateZ = 40 + Math.sin(idlePhase * Math.PI * 2 + Math.PI / 3) * 10;
    const translateZ = idleElapsedMs <= 0 ? entryTranslateZ : idleTranslateZ;
    const scale = idleElapsedMs <= 0 ? overshootScale : 1;
  ...
  return (
    <section className="relative min-h-screen overflow-x-hidden bg-[#030508] lg:h-screen lg:overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_16%_26%,rgba(85,157,224,0.18),transparent_28%),radial-gradient(circle_at_78%_18%,rgba(135,191,255,0.1),transparent_24%),linear-gradient(115deg,#020406_0%,#07111b_42%,#030508_100%)]" />
      <div className="absolute inset-x-0 bottom-0 h-[30vh] bg-[linear-gradient(180deg,transparent_0%,rgba(4,9,14,0.52)_36%,rgba(3,7,11,0.96)_100%)]" />
