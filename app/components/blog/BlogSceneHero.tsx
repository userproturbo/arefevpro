...
export default function BlogSceneHero({ children }: BlogSceneHeroProps) {
  ...
  useEffect(() => {
    ...
    const translateZ = 18 + ramp * 20 + (secondaryWave + 1) * 7;
    const rotateZ = wave * (0.5 + 0.55 * ramp);
    const scaleOvershoot = 1 + (1 - entryProgress) * 0.06;
    const idleScale = 1 + wave * 0.01 + (isHovered ? 0.02 : 0);
    const scale = idleElapsedMs <= 0 ? scaleOvershoot : idleScale;
    const brightness = 1 + ramp * (isHovered ? 0.09 : 0.04);
  ...
  return (
    <section className="relative min-h-screen overflow-x-hidden bg-[#050507] lg:h-screen lg:overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_26%,rgba(255,126,92,0.16),transparent_30%),radial-gradient(circle_at_76%_18%,rgba(255,217,128,0.08),transparent_24%),linear-gradient(118deg,#050507_0%,#140b0d_42%,#060608_100%)]" />
      <div className="absolute inset-x-0 bottom-0 h-[30vh] bg-[linear-gradient(180deg,transparent_0%,rgba(13,7,9,0.52)_36%,rgba(9,5,7,0.96)_100%)]" />
